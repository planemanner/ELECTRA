...
def get_layer_decayed_lrs(lrs, pct, warmup_steps, total_steps):
    ...
    for lr in lrs:
        layer_lrs += [
            linear_warmup_and_then_decay(pct=pct, lr_max=lr, total_steps=total_steps, warmup_steps=warmup_steps)]
    return layer_lrs
    ...
    for epoch in range(args.epochs):
        for idx, data in enumerate(Train_loader):
            ...
            with torch.no_grad():
                if train_iter % 50 == 0:
                    print(
                        f"{epoch + 1} / {args.epochs}, {idx + 1} / {Train_loader.__len__()}, Train Loss : {loss.item()}")
            train_iter += 1
...
class HPO_BERT:
    ...
    def __call__(self, trial):
        ...
        self.args.batch_size = trial.suggest_int("bs", 4, 256, log=True)

        All_tasks = ["CoLA", "SST-2", "MRPC", "QQP", "MNLI", "QNLI", "WNLI", "RTE"]
        ...
        for task, epochs in zip(All_tasks, Task_wise_epochs):
            args.task = task
            args.epochs = epochs
            avg_results += [fine_tuner(args, trial)]

        return np.mean(avg_results)


if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument("--pretrained_model_weight_path", type=str,
                        default="/vision/7032593/NLP/ELECTRA/check_points/DISC_ITER_900000_LM_MODEL.pth")
    ...
    num_trials = 200
    study_name = f"GLUE_HPO_{args.seed}"
    ...
    print("  Best Acc or F1 is : ", trial.value)
    """
    key : parameter name, value : the value of parameter
    """
